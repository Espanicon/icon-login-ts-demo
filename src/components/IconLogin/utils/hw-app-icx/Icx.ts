...
export default class Icx {
   ...
   */
  async getAppConfiguration(): Promise<GetAppConfiguration> {
    return this.transport.send(0xe0, 0x06, 0x00, 0x00).then((response: any) => {
      const result: GetAppConfiguration = {};
  ...
  setTestPrivateKey(privateKeyHex: string) {
    ...
    for (let i = 0; i < privateKeyHex.length; i += 2) {
      /*
       * String.prototype.substr(start, length) is deprecated. For the case
       * of this function the code has been changed like this:
       *  - data[i / 2] = parseInt(privateKeyHex.substr(i, 2), 16);
       *  - data[i / 2] = parseInt(privateKeyHex.slice(i, i+2), 16);
       *
       *  With this change the logic should still maintain which is to return
       *  a two characted slice of the string 'privateKeyHex' using 'i' as
       *  the start and 'i+2' as the end.
       */
      //
      data[i / 2] = parseInt(privateKeyHex.slice(i, i + 2), 16);
    }
